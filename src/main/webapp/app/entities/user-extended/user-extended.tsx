          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('note1')}>
                  Note 1 <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('note2')}>
                  Note 2 <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('note3')}>
                  Note 3 <FontAwesomeIcon icon="sort" />
                </th>
                <th>
            ...
            <tbody>
              {userExtendedList.map((userExtended, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{userExtended.username}</td>
                  <td>{userExtended.provincia}</td>
                  <td>{userExtended.note1}</td>
                  <td>{userExtended.note2}</td>
                  <td>{userExtended.note3}</td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/user-extended/${userExtended.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/user-extended/${userExtended.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
